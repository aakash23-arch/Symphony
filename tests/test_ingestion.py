...
def test_websocket_disconnect_leaves_session_terminal(client):
    """A mid-stream disconnect ends the session cleanly, with no exception."""
    from voiceshield.api.runtime import reset_runtime

    runtime = reset_runtime()
    session_id = runtime.sessions.create(source_type="ws").session_id

    payload = pcm_bytes(tone(0.5))
    with client.websocket_connect(f"/v1/sessions/{session_id}/audio") as ws:
        ws.send_text(json.dumps({
            "type": "audio.header", "sample_rate": FIXTURE_RATE,
            "channels": 1, "encoding": PCM_S16LE,
        }))
        ws.send_bytes(payload[: len(payload) // 2])
        # Drop the connection mid-stream.

    record = runtime.sessions.get(session_id)
    assert record.is_terminal or record.state is SessionState.CREATED


def _drain_until_close(ws, limit: int = 8) -> dict:
    """Read frames until the server's close frame, and return it."""
    for _ in range(limit):
        message = ws.receive()
        if message.get("type") == "websocket.close":
            return message
    raise AssertionError("server never closed the connection")


def test_websocket_rejects_text_after_header(client):
    """§8.1: a non-binary message after the header closes with 4400."""
    from voiceshield.api.runtime import reset_runtime
    ...
    with client.websocket_connect(f"/v1/sessions/{session_id}/audio") as ws:
        ...
        ws.send_text(json.dumps({"type": "control", "action": "boost"}))
        closed = _drain_until_close(ws)

    assert closed["code"] == 4400
    assert runtime.sessions.get(session_id).reason == AUDIO_PROTOCOL_VIOLATION


def test_websocket_rejects_malformed_header(client):
    """A header that is not JSON closes with 4400 and never starts capture."""
    from voiceshield.api.runtime import reset_runtime
    ...
    assert error["code"] == AUDIO_PROTOCOL_VIOLATION
    assert closed["code"] == 4400


def test_websocket_rejects_unsupported_declared_encoding(client):
    """The declared header is validated, never trusted."""
    from voiceshield.api.runtime import reset_runtime
...
def test_websocket_unknown_session_is_closed(client):
    from voiceshield.api.runtime import reset_runtime

    reset_runtime()
    with client.websocket_connect("/v1/sessions/nope/audio") as ws:
        message = ws.receive_json()
    assert message["code"] == "SESSION_NOT_FOUND"


def test_websocket_streams_audio_and_acks(client):
    from voiceshield.api.runtime import reset_runtime

    runtime = reset_runtime()
    session_id = runtime.sessions.create(source_type="ws").session_id

    payload = pcm_bytes(voiced(1.0, f0=140.0))
    chunk = len(payload) // 20
    ...
    with client.websocket_connect(f"/v1/sessions/{session_id}/audio") as ws:
        ...
        for i in range(20):
            ws.send_bytes(payload[i * chunk : (i + 1) * chunk])
        ack = ws.receive_json()
